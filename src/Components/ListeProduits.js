...
export default function ListeProduits({ produits, produitsLoaded }) {
  ...
  const produitsFiltres = selectedCategorie
    ? produits.filter(
        (p) => p.categorie?.nom?.toLowerCase() === selectedCategorie.toLowerCase()
      )
    : produits;
  ...
  return (
    <div className="container mt-5">
      <h2 className="text-center mb-4">
        {selectedCategorie ? `Produits : ${selectedCategorie}` : "Nos produits disponibles"}
      </h2>

      {!produitsLoaded ? (
        <p className="text-center text-muted">Chargement des produits...</p>
      ) : produitsFiltres.length === 0 ? (
        <p className="text-center text-muted">Aucun produit trouvé pour cette catégorie.</p>
      ) : (
        <div className="row">
